import React, { useState, useEffect } from 'react';
  import { Permission, PermissionInput, PermissionService } from '@/services/permission.service';
  ...
  const PermissionForm: React.FC<PermissionFormProps> = ({ id, onSuccess, onError }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await PermissionService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await PermissionService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.permission.title')}
  ...
  id="name"
  name="name"
  label={t('models.permission.fields.name')}
...
/>
  ...
  id="description"
  name="description"
  label={t('models.permission.fields.description')}
