import React, { useState, useEffect } from 'react';
  import { MaterialSupplier, MaterialSupplierInput, MaterialSupplierService } from '@/services/materialSupplier.service';
  ...
  interface MaterialSupplierFormProps {
    id?: string;
    onSuccess?: () => void;
    onError?: (error: string) => void;
  }
  
  const MaterialSupplierForm: React.FC<MaterialSupplierFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      setLoading(true);
      const data = await MaterialSupplierService.getById(id!);
      ...
      formattedData.supplierCode = data.supplierCode;
      formattedData.price = data.price;
      formattedData.leadTimeDays = data.leadTimeDays;
      formattedData.isPreferred = data.isPreferred;
      formattedData.minOrderQuantity = data.minOrderQuantity;
      if (data.lastPurchaseDate && (data.lastPurchaseDate instanceof Date || typeof data.lastPurchaseDate === 'string')) {
            formattedData.lastPurchaseDate = new Date(data.lastPurchaseDate).toISOString().substring(0, 10);
          } else {
            formattedData.lastPurchaseDate = data.lastPurchaseDate;
          }
    ...
    if (!formData.price) errors.price = 'El campo price es requerido';
    ...
    dataToSubmit.supplierCode = formData.supplierCode;
    dataToSubmit.price = formData.price;
    dataToSubmit.leadTimeDays = formData.leadTimeDays;
    dataToSubmit.isPreferred = formData.isPreferred;
    dataToSubmit.minOrderQuantity = formData.minOrderQuantity;
    dataToSubmit.lastPurchaseDate = formData.lastPurchaseDate;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await MaterialSupplierService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await MaterialSupplierService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.materialSupplier.title')}
    ...
    <label htmlFor="materialId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.materialSupplier.fields.material')}    
    ...
    <label htmlFor="supplierId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.materialSupplier.fields.supplier')}    
    <span className="text-red-500 ml-1">*</span>
    </label>
    <GenericSelect
      id="supplierId"
  ...
  id="supplierCode"
  name="supplierCode"
  label={t('models.materialSupplier.fields.supplierCode')}
...
/>
  ...
  id="price"
  name="price"
  label={t('models.materialSupplier.fields.price')}
...
/>
  ...
  id="leadTimeDays"
  name="leadTimeDays"
  label={t('models.materialSupplier.fields.leadTimeDays')}
...
/>
  ...
  error={fieldErrors.isPreferred}
/>
      {/* Campo: minOrderQuantity */}
          <GenericInput
  type="number"
  id="minOrderQuantity"
  name="minOrderQuantity"
  label={t('models.materialSupplier.fields.minOrderQuantity')}

  value={formData.minOrderQuantity?.toString() ?? ''}
  onChange={handleChange}
  
  error={fieldErrors.minOrderQuantity}
/>
      {/* Campo: lastPurchaseDate */}
          <GenericInput
  type="datetime-local"
  id="lastPurchaseDate"
  name="lastPurchaseDate"
  label={t('models.materialSupplier.fields.lastPurchaseDate')}

  value={formData.lastPurchaseDate ? new Date(formData.lastPurchaseDate).toISOString().slice(0, 16) : ''}
  onChange={handleChange}
  
  error={fieldErrors.lastPurchaseDate}
/>
