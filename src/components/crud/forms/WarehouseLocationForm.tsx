import React, { useState, useEffect } from 'react';
  import { WarehouseLocation, WarehouseLocationInput, WarehouseLocationService } from '@/services/warehouseLocation.service';
  ...
  const WarehouseLocationForm: React.FC<WarehouseLocationFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      setLoading(true);
      const data = await WarehouseLocationService.getById(id!);
    ...
    const validateForm = (): boolean => {
      ...
      if (!formData.warehouseId) errors.warehouseId = 'El campo warehouse es requerido';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await WarehouseLocationService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await WarehouseLocationService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.warehouseLocation.title')}
    ...
    <label htmlFor="warehouseId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.warehouseLocation.fields.warehouse')}    
  ...
  id="code"
  name="code"
  label={t('models.warehouseLocation.fields.code')}
...
/>
  ...
  id="description"
  name="description"
  label={t('models.warehouseLocation.fields.description')}
