import React, { useState, useEffect } from 'react';
  import { CustomerAddress, CustomerAddressInput, CustomerAddressService } from '@/services/customerAddress.service';
  ...
  const CustomerAddressForm: React.FC<CustomerAddressFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      setLoading(true);
      const data = await CustomerAddressService.getById(id!);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await CustomerAddressService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await CustomerAddressService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.customerAddress.title')}
    ...
    <label htmlFor="customerId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.customerAddress.fields.customer')}    
    ...
    <label htmlFor="addressType" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.customerAddress.fields.addressType')}
  ...
  id="street"
  name="street"
  label={t('models.customerAddress.fields.street')}
...
/>
  ...
  id="city"
  name="city"
  label={t('models.customerAddress.fields.city')}
...
/>
  ...
  id="postalCode"
  name="postalCode"
  label={t('models.customerAddress.fields.postalCode')}
...
/>
  ...
  id="region"
  name="region"
  label={t('models.customerAddress.fields.region')}
...
/>
  ...
  id="country"
  name="country"
  label={t('models.customerAddress.fields.country')}
