import React, { useState, useEffect } from 'react';
  import { ProductLocation, ProductLocationInput, ProductLocationService } from '@/services/productLocation.service';
  ...
  const ProductLocationForm: React.FC<ProductLocationFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      setLoading(true);
      const data = await ProductLocationService.getById(id!);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await ProductLocationService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await ProductLocationService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.productLocation.title')}
    ...
    <label htmlFor="productId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.productLocation.fields.product')}    
    ...
    <label htmlFor="locationId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.productLocation.fields.location')}    
    <span className="text-red-500 ml-1">*</span>
    </label>
    <GenericSelect
      id="locationId"
  ...
  id="quantity"
  name="quantity"
  label={t('models.productLocation.fields.quantity')}
