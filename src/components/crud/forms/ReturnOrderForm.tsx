import React, { useState, useEffect } from 'react';
  import { ReturnOrder, ReturnOrderInput, ReturnOrderService } from '@/services/returnOrder.service';
  ...
  const ReturnOrderForm: React.FC<ReturnOrderFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      setLoading(true);
      const data = await ReturnOrderService.getById(id!);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await ReturnOrderService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await ReturnOrderService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.returnOrder.title')}
  ...
  id="reference"
  name="reference"
  label={t('models.returnOrder.fields.reference')}
...
/>
    ...
    <label htmlFor="saleId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.returnOrder.fields.sale')}    
    ...
    <label htmlFor="customerId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.returnOrder.fields.customer')}    
  ...
  id="date"
  name="date"
  label={t('models.returnOrder.fields.date')}
...
/>
  ...
  id="reason"
  name="reason"
  label={t('models.returnOrder.fields.reason')}
...
/>
    ...
    <label htmlFor="status" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.returnOrder.fields.status')}
    ...
    <BaseSelect
      ...
      name="status"
      options={[
        { id: 'PENDING         // Pendiente de procesar', name: 'PENDING         // Pendiente de procesar' },
        { id: 'APPROVED        // Aprobada', name: 'APPROVED        // Aprobada' },
        { id: 'REJECTED        // Rechazada', name: 'REJECTED        // Rechazada' },
        { id: 'COMPLETED       // Completada', name: 'COMPLETED       // Completada' },
        { id: 'REFUNDED        // Reembolsada', name: 'REFUNDED        // Reembolsada' }
      ]}
      valueKey="id"
      labelKey="name"
  ...
  id="totalAmount"
  name="totalAmount"
  label={t('models.returnOrder.fields.totalAmount')}
...
/>
  ...
  id="notes"
  name="notes"
  label={t('models.returnOrder.fields.notes')}
