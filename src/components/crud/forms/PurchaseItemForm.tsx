import React, { useState, useEffect } from 'react';
  import { PurchaseItem, PurchaseItemInput, PurchaseItemService } from '@/services/purchaseItem.service';
  ...
  const PurchaseItemForm: React.FC<PurchaseItemFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      ...
      formattedData.receivedQuantity = data.receivedQuantity;
    ...
    const validateForm = (): boolean => {
      ...
      if (!formData.purchaseId) errors.purchaseId = 'El campo purchase es requerido';
    ...
    const prepareDataForSubmission = (): any => {
      ...
      if (formData.purchaseId) {
        dataToSubmit.purchaseId = formData.purchaseId;
      }
    ...
    dataToSubmit.receivedQuantity = formData.receivedQuantity;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await PurchaseItemService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await PurchaseItemService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.purchaseItem.title')}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Campo: purchase */}
          <div>
    <label htmlFor="purchaseId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.purchaseItem.fields.purchase')}    
    ...
    <GenericSelect
      ...
      onChange={(value) => handleSelectChange('purchaseId', parseInt(value))}
      url="/purchases"
      labelKey="id"
      valueKey="id"
      required
    ...
    <label htmlFor="materialId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.purchaseItem.fields.material')}    
    <span className="text-red-500 ml-1">*</span>
    </label>
    <GenericSelect
      id="materialId"
  ...
  id="quantity"
  name="quantity"
  label={t('models.purchaseItem.fields.quantity')}
...
/>
  ...
  id="unitPrice"
  name="unitPrice"
  label={t('models.purchaseItem.fields.unitPrice')}
...
/>
  ...
  id="taxRate"
  name="taxRate"
  label={t('models.purchaseItem.fields.taxRate')}
...
/>
  ...
  id="total"
  name="total"
  label={t('models.purchaseItem.fields.total')}
...
/>
  ...
  label={t('models.purchaseItem.fields.discountPercent')}
...
/>
  ...
  id="notes"
  name="notes"
  label={t('models.purchaseItem.fields.notes')}
