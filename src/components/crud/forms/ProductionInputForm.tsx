import React, { useState, useEffect } from 'react';
  import { ProductionInput, ProductionInputInput, ProductionInputService } from '@/services/productionInput.service';
  ...
  const ProductionInputForm: React.FC<ProductionInputFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      setLoading(true);
      const data = await ProductionInputService.getById(id!);
      ...
      formattedData.unitCost = data.unitCost;
    ...
    dataToSubmit.unitCost = formData.unitCost;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await ProductionInputService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await ProductionInputService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.productionInput.title')}
    ...
    <label htmlFor="productionId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.productionInput.fields.production')}    
    ...
    <label htmlFor="materialId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.productionInput.fields.material')}    
    <span className="text-red-500 ml-1">*</span>
    </label>
    <GenericSelect
      id="materialId"
  ...
  id="quantity"
  name="quantity"
  label={t('models.productionInput.fields.quantity')}
...
/>
  ...
  id="unitCost"
  name="unitCost"
  label={t('models.productionInput.fields.unitCost')}
...
/>
    ...
    <label htmlFor="locationId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.productionInput.fields.location')}    
