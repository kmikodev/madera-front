import React, { useState, useEffect } from 'react';
  import { PasswordHistory, PasswordHistoryInput, PasswordHistoryService } from '@/services/passwordHistory.service';
  ...
  interface PasswordHistoryFormProps {
    id?: string;
    onSuccess?: () => void;
    onError?: (error: string) => void;
  }
  
  const PasswordHistoryForm: React.FC<PasswordHistoryFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      setLoading(true);
      const data = await PasswordHistoryService.getById(id!);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await PasswordHistoryService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await PasswordHistoryService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.passwordHistory.title')}
    ...
    <label htmlFor="userId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.passwordHistory.fields.user')}    
  ...
  id="password"
  name="password"
  label={t('models.passwordHistory.fields.password')}
...
/>
  ...
  export default PasswordHistoryForm;