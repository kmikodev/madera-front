import React, { useState, useEffect } from 'react';
  import { SupplierContact, SupplierContactInput, SupplierContactService } from '@/services/supplierContact.service';
  ...
  const SupplierContactForm: React.FC<SupplierContactFormProps> = ({ id, onSuccess, onError }) => {
   ...
   const loadData = async () => {
    try {
      setLoading(true);
      const data = await SupplierContactService.getById(id!);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (id) {
          ...
          await SupplierContactService.update(id, dataToSubmit);
        } else {
          // Crear nuevo registro
          await SupplierContactService.create(dataToSubmit);
        }
    ...
    return (
      <div className="max-w-lg mx-auto p-4">
        <h2 className="text-xl font-bold mb-4 text-secondary-900 dark:text-white">
          {id ? t('commons.forms.edit') : t('commons.forms.create')} {t('models.supplierContact.title')}
    ...
    <label htmlFor="supplierId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">
      {t('models.supplierContact.fields.supplier')}    
  ...
  id="name"
  name="name"
  label={t('models.supplierContact.fields.name')}
...
/>
  ...
  id="position"
  name="position"
  label={t('models.supplierContact.fields.position')}
...
/>
  ...
  id="phone"
  name="phone"
  label={t('models.supplierContact.fields.phone')}
...
/>
  ...
  id="email"
  name="email"
  label={t('models.supplierContact.fields.email')}
