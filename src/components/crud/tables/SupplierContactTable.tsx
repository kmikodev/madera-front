  ...
  import { SupplierContact, SupplierContactService } from '@/services/supplierContact.service';
  ...
  const SupplierContactTable: React.FC<SupplierContactTableProps> = ({ onEdit, onDelete }) => {
    const [data, setData] = useState<SupplierContact[]>([]);
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const result = await SupplierContactService.getAll();
        setData(result || []);
        setError(null);
      } catch (err: any) {
    ...
    const handleDelete = async (id: number) => {
      if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
        try {
          await SupplierContactService.delete(id);
    ...
    return (
      <div className="overflow-x-auto bg-white dark:bg-secondary-800 rounded-lg shadow">
        <div className="flex justify-between p-2 border-b border-gray-200 dark:border-secondary-700">
          <h3 className="font-semibold text-lg text-secondary-900 dark:text-white">SupplierContacts</h3>
          <button 
            onClick={() => loadData()}
