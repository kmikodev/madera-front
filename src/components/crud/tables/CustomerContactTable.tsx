  ...
  import { CustomerContact, CustomerContactService } from '@/services/customerContact.service';
  ...
  const CustomerContactTable: React.FC<CustomerContactTableProps> = ({ onEdit, onDelete }) => {
    const [data, setData] = useState<CustomerContact[]>([]);
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const result = await CustomerContactService.getAll();
        setData(result || []);
        setError(null);
      } catch (err: any) {
    ...
    return (
      <div className="overflow-x-auto bg-white dark:bg-secondary-800 rounded-lg shadow">
        <div className="flex justify-between p-2 border-b border-gray-200 dark:border-secondary-700">
          <h3 className="font-semibold text-lg text-secondary-900 dark:text-white">CustomerContacts</h3>
          <button 
            onClick={() => loadData()}
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-secondary-700">
          ...
          <tbody className="bg-white dark:bg-secondary-800 divide-y divide-gray-200 dark:divide-secondary-700">
            {data.map((item) => (
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.position, 'String', 'position')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.isPrimary, 'Boolean', 'isPrimary')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.department, 'String', 'department')}
                  </td>
