  ...
  import { AccountingEntry, AccountingEntryService } from '@/services/accountingEntry.service';
  ...
  const AccountingEntryTable: React.FC<AccountingEntryTableProps> = ({ onEdit, onDelete }) => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const result = await AccountingEntryService.getAll();
        setData(result || []);
        setError(null);
      } catch (err: any) {
    ...
    const handleDelete = async (id: number) => {
      if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
        try {
          await AccountingEntryService.delete(id);
    ...
    return (
      <div className="overflow-x-auto bg-white dark:bg-secondary-800 rounded-lg shadow">
        <div className="flex justify-between p-2 border-b border-gray-200 dark:border-secondary-700">
          <h3 className="font-semibold text-lg text-secondary-900 dark:text-white">AccountingEntrys</h3>
          <button 
            onClick={() => loadData()}
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-secondary-700">
          ...
          <tbody className="bg-white dark:bg-secondary-800 divide-y divide-gray-200 dark:divide-secondary-700">
            {data.map((item) => (
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.isAdjustment, 'Boolean', 'isAdjustment')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.payment, 'Payment', 'payment')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.paymentId, 'Int', 'paymentId')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.entryType, 'String', 'entryType')}
                  </td>
