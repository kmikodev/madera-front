  ...
  import { SepaRemittance, SepaRemittanceService } from '@/services/sepaRemittance.service';
  import { ApiError } from '@/services/apiClient';
  
  interface SepaRemittanceTableProps {
    onEdit?: (id: number) => void;
    onDelete?: () => void;
  }
  
  const SepaRemittanceTable: React.FC<SepaRemittanceTableProps> = ({ onEdit, onDelete }) => {
    const [data, setData] = useState<SepaRemittance[]>([]);
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const result = await SepaRemittanceService.getAll();
        setData(result || []);
        setError(null);
      } catch (err: any) {
    ...
    const handleDelete = async (id: number) => {
      if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
        try {
          await SepaRemittanceService.delete(id);
    ...
    return (
      <div className="overflow-x-auto bg-white dark:bg-secondary-800 rounded-lg shadow">
        <div className="flex justify-between p-2 border-b border-gray-200 dark:border-secondary-700">
          <h3 className="font-semibold text-lg text-secondary-900 dark:text-white">SepaRemittances</h3>
          <button 
            onClick={() => loadData()}
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-secondary-700">
          ...
          <tbody className="bg-white dark:bg-secondary-800 divide-y divide-gray-200 dark:divide-secondary-700">
            {data.map((item) => (
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.generationDate, 'DateTime', 'generationDate')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.submissionDate, 'DateTime', 'submissionDate')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.status, 'SepaStatus', 'status')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.totalAmount, 'Decimal', 'totalAmount')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.messageId, 'String', 'messageId')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
  ...
  export default SepaRemittanceTable;
