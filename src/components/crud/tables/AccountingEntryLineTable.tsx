  ...
  import { AccountingEntryLine, AccountingEntryLineService } from '@/services/accountingEntryLine.service';
  import { ApiError } from '@/services/apiClient';
  
  interface AccountingEntryLineTableProps {
    onEdit?: (id: number) => void;
    onDelete?: () => void;
  }
  
  const AccountingEntryLineTable: React.FC<AccountingEntryLineTableProps> = ({ onEdit, onDelete }) => {
    const [data, setData] = useState<AccountingEntryLine[]>([]);
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const result = await AccountingEntryLineService.getAll();
        setData(result || []);
        setError(null);
      } catch (err: any) {
    ...
    const handleDelete = async (id: number) => {
      if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
        try {
          await AccountingEntryLineService.delete(id);
    ...
    return (
      <div className="overflow-x-auto bg-white dark:bg-secondary-800 rounded-lg shadow">
        <div className="flex justify-between p-2 border-b border-gray-200 dark:border-secondary-700">
          <h3 className="font-semibold text-lg text-secondary-900 dark:text-white">AccountingEntryLines</h3>
          <button 
            onClick={() => loadData()}
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-secondary-700">
          ...
          <tbody className="bg-white dark:bg-secondary-800 divide-y divide-gray-200 dark:divide-secondary-700">
            {data.map((item) => (
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.entry, 'AccountingEntry', 'entry')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.entryId, 'Int', 'entryId')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.description, 'String', 'description')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.debitAccount, 'AccountingAccount', 'debitAccount')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.debitAccountId, 'Int', 'debitAccountId')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.creditAccount, 'AccountingAccount', 'creditAccount')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.creditAccountId, 'Int', 'creditAccountId')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
  ...
  export default AccountingEntryLineTable;
