  ...
  import { QualityDefect, QualityDefectService } from '@/services/qualityDefect.service';
  import { ApiError } from '@/services/apiClient';
  
  interface QualityDefectTableProps {
    onEdit?: (id: number) => void;
    onDelete?: () => void;
  }
  
  const QualityDefectTable: React.FC<QualityDefectTableProps> = ({ onEdit, onDelete }) => {
    const [data, setData] = useState<QualityDefect[]>([]);
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const result = await QualityDefectService.getAll();
        setData(result || []);
        setError(null);
      } catch (err: any) {
    ...
    const handleDelete = async (id: number) => {
      if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
        try {
          await QualityDefectService.delete(id);
    ...
    return (
      <div className="overflow-x-auto bg-white dark:bg-secondary-800 rounded-lg shadow">
        <div className="flex justify-between p-2 border-b border-gray-200 dark:border-secondary-700">
          <h3 className="font-semibold text-lg text-secondary-900 dark:text-white">QualityDefects</h3>
          <button 
            onClick={() => loadData()}
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-secondary-700">
          ...
          <tbody className="bg-white dark:bg-secondary-800 divide-y divide-gray-200 dark:divide-secondary-700">
            {data.map((item) => (
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.qualityControlId, 'Int', 'qualityControlId')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.qualityControl, 'QualityControl', 'qualityControl')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.defectType, 'String', 'defectType')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.description, 'String', 'description')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.severity, 'DefectSeverity', 'severity')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.quantity, 'Int', 'quantity')}
                  </td>
  ...
  export default QualityDefectTable;
