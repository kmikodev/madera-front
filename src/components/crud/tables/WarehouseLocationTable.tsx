  ...
  import { WarehouseLocation, WarehouseLocationService } from '@/services/warehouseLocation.service';
  ...
  const WarehouseLocationTable: React.FC<WarehouseLocationTableProps> = ({ onEdit, onDelete }) => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const result = await WarehouseLocationService.getAll();
        setData(result || []);
        setError(null);
      } catch (err: any) {
    ...
    return (
      <div className="overflow-x-auto bg-white dark:bg-secondary-800 rounded-lg shadow">
        <div className="flex justify-between p-2 border-b border-gray-200 dark:border-secondary-700">
          <h3 className="font-semibold text-lg text-secondary-900 dark:text-white">WarehouseLocations</h3>
          <button 
            onClick={() => loadData()}
