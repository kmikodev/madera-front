  ...
  import { MaterialSupplier, MaterialSupplierService } from '@/services/materialSupplier.service';
  import { ApiError } from '@/services/apiClient';
  
  interface MaterialSupplierTableProps {
    onEdit?: (id: string) => void;
    onDelete?: () => void;
  }
  
  const MaterialSupplierTable: React.FC<MaterialSupplierTableProps> = ({ onEdit, onDelete }) => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const result = await MaterialSupplierService.getAll();
        setData(result || []);
        setError(null);
      } catch (err: any) {
    ...
    const handleDelete = async (id: string) => {
      if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
        try {
          await MaterialSupplierService.delete(id);
    ...
    return (
      <div className="overflow-x-auto bg-white dark:bg-secondary-800 rounded-lg shadow">
        <div className="flex justify-between p-2 border-b border-gray-200 dark:border-secondary-700">
          <h3 className="font-semibold text-lg text-secondary-900 dark:text-white">MaterialSuppliers</h3>
          <button 
            onClick={() => loadData()}
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-secondary-700">
          ...
          <tbody className="bg-white dark:bg-secondary-800 divide-y divide-gray-200 dark:divide-secondary-700">
            {data.map((item) => (
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.supplierCode, 'String', 'supplierCode')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.price, 'Decimal', 'price')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.leadTimeDays, 'Int', 'leadTimeDays')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.isPreferred, 'Boolean', 'isPreferred')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.minOrderQuantity, 'Decimal', 'minOrderQuantity')}
                  </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                    {formatValue(item.lastPurchaseDate, 'DateTime', 'lastPurchaseDate')}
                  </td>
  ...
  export default MaterialSupplierTable;
