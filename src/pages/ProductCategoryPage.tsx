  ...
  import ProductCategoryForm from '@/components/crud/forms/ProductCategoryForm';
  import ProductCategoryTable from '@/components/crud/tables/ProductCategoryTable';
  ...
  const ProductCategoryPage: React.FC = () => {
    ...
    const handleEdit = (id: number) => {
      navigate(`/productCategorys/edit/${id}`);
    };
  
    const handleSuccess = () => {
      setRefreshTrigger(prev => prev + 1);
      navigate('/productCategorys');
    };
    ...
    const EditView = () => {
      ...
      return (
        <>
          ...
          {id && <ProductCategoryForm id={id} onSuccess={handleSuccess} />}
        </>
      );
    };
            ...
            element={
              <>
                <div className="mb-6 flex justify-between items-center">
                  <h2 className="text-2xl font-bold text-secondary-900 dark:text-white">{t('models.productCategory.title')}</h2>
                  <button
                    onClick={() => navigate('/productCategorys/new')}
                  ...
                  >
                    {t('commons.forms.new')} {t('models.productCategory.title')}
                  </button>
                </div>
